    ...
    tpl->SetClassName(Nan::New("Error").ToLocalChecked());
           ...
           Nan::SetMethod(tpl, "clear", Clear, nodegitExternal);
            Nan::SetPrototypeMethod(tpl, "last", Last, nodegitExternal);
             Nan::SetMethod(tpl, "set", Set, nodegitExternal);
...
NAN_METHOD(GitError::Set) {
  ...
  if (info.Length() == 1 || !info[1]->IsString()) {
    return Nan::ThrowError("String fmt is required.");
  }
...
// start convert_from_v8 block
  const char * from_fmt = NULL;

  Nan::Utf8String fmt(Nan::To<v8::String>(info[1]).ToLocalChecked());
  // malloc with one extra byte so we can add the terminating null character C-strings expect:
  from_fmt = (const char *) malloc(fmt.length() + 1);
  // copy the characters from the nodejs string into our C-string (used instead of strdup or strcpy because nulls in
  // the middle of strings are valid coming from nodejs):
  memcpy((void *)from_fmt, *fmt, fmt.length());
  // ensure the final byte of our new string is null, extra casts added to ensure compatibility with various C types
  // used in the nodejs binding generation:
  memset((void *)(((char *)from_fmt) + fmt.length()), 0, 1);
// end convert_from_v8 block
