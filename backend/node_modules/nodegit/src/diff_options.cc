  ...
  // unpack the data into the correct fields
         {
          ...
          if (!maybeNumber.IsEmpty() && !maybeNumber->IsUndefined() && !maybeNumber->IsNull()) {
           ...
           {
          v8::Local<v8::Value> maybeNumber = nodegit::safeGetField(inputObj, "oidType");
          if (!maybeNumber.IsEmpty() && !maybeNumber->IsUndefined() && !maybeNumber->IsNull()) {
            if (!maybeNumber->IsNumber()) {
              return {
            ...
            output->raw->oid_type = static_cast<git_oid_t>(maybeNumber->Int32Value(Nan::GetCurrentContext()).FromJust());
          }
        }
           {
