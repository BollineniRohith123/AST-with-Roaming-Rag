// This is a generated file, modify: generate/templates/templates/class_content.cc

#include <nan.h>
#include <string.h>

extern "C" {
  #include <git2.h>
 }

#include "../include/nodegit.h"
#include "../include/lock_master.h"
#include "../include/functions/copy.h"
#include "../include/diff_parse_options.h"
#include "nodegit_wrapper.cc"

 
#include <iostream>

using namespace std;
using namespace v8;
using namespace node;

  GitDiffParseOptions::~GitDiffParseOptions() {
    // this will cause an error if you have a non-self-freeing object that also needs
    // to save values. Since the object that will eventually free the object has no
    // way of knowing to free these values.
   }

  void GitDiffParseOptions::InitializeComponent(v8::Local<v8::Object> target, nodegit::Context *nodegitContext) {
    Nan::HandleScope scope;

    v8::Local<v8::External> nodegitExternal = Nan::New<v8::External>(nodegitContext);
    v8::Local<FunctionTemplate> tpl = Nan::New<FunctionTemplate>(JSNewFunction, nodegitExternal);

    tpl->InstanceTemplate()->SetInternalFieldCount(2);
    tpl->SetClassName(Nan::New("DiffParseOptions").ToLocalChecked());

         Nan::SetPrototypeMethod(tpl, "version", Version, nodegitExternal);
         Nan::SetPrototypeMethod(tpl, "oidType", OidType, nodegitExternal);
  
    InitializeTemplate(tpl);

    v8::Local<Function> constructor_template = Nan::GetFunction(tpl).ToLocalChecked();
    nodegitContext->SaveToPersistent("GitDiffParseOptions::Template", constructor_template);
    Nan::Set(target, Nan::New("DiffParseOptions").ToLocalChecked(), constructor_template);
  }

      // start field block
    NAN_METHOD(GitDiffParseOptions::Version) {
      v8::Local<v8::Value> v8ConversionSlot;

            unsigned int
           version =
          Nan::ObjectWrap::Unwrap<GitDiffParseOptions>(info.This())->GetValue()->version;
 // start convert_to_v8 block
     v8ConversionSlot = Nan::New<Number>( version);
  // end convert_to_v8 block
      info.GetReturnValue().Set(v8ConversionSlot);
    }
    // end field block
     // start field block
    NAN_METHOD(GitDiffParseOptions::OidType) {
      v8::Local<v8::Value> v8ConversionSlot;

            git_oid_t
           oid_type =
          Nan::ObjectWrap::Unwrap<GitDiffParseOptions>(info.This())->GetValue()->oid_type;
 // start convert_to_v8 block
     v8ConversionSlot = Nan::New<Number>( oid_type);
  // end convert_to_v8 block
      info.GetReturnValue().Set(v8ConversionSlot);
    }
    // end field block
  // force base class template instantiation, to make sure we get all the
// methods, statics, etc.
template class NodeGitWrapper<GitDiffParseOptionsTraits>;
 